...
conf_mat <- function(data.long, col1, col2 = col1, axis.col = NULL, axis1 = NULL, axis2 = NULL, key = TRUE, num1 = "full", num2 = "sample", spec.abb = NULL, abb.end = NULL, axes.same = TRUE, xlab = paste(col1), ylab = paste(col2), sp.list = "present", sp.exc = NULL, subset.col = NULL, subset.lev = NULL, palette = "matlab.like", grid = FALSE, grid.sp = 5, grid.col = "grey40", type = "CvID") {
  ...
  # grid / grid.sp / grid.col - if a grid should be plotted, how many species should it include, and what colour should it be
  # type - either "CvID" where a consensus is compared with a set of IDs, or "CvC", where two consensus sets are compared. 
  ...
  if (type == "CvID") {
    ...
    image(t(conf$table), col =c("grey70", do.call(palette, list(max(conf$table)))), axes = FALSE, add = TRUE)
  } else {
    stop(paste("type", type, "not recognised"))
  }
  # add labels
  ...
  if (key) {
    ...
    if (type == "CvID") {
      ...
      names.key <- seq(0, max(conf$table), by = 10)
      key <- rep(1, (max(conf$table) + 1))
      barplot(key, main = "\nNumber of\nSpecimens", horiz = TRUE, space = 0, border = NA, col = c("grey70", do.call(palette, list(max(conf$table)))), fg = "white", cex.main = 1.1, font.main = 1, xaxt = "n", yaxt = "n")
      axis(4, at = seq(1, max(conf$table), by = 10), names.key, las = 1, mgp = c(0, 1, 0), xaxt = "n", cex.axis = 1, tick = FALSE)
    } else {
      stop(paste("type", type, "not recognised"))
    }
  }
